...
func renderCardMap(card *core.Card, sessionKey string) map[string]any {
	...
	if helpElements, ok := renderHelpCardElements(card, sessionKey); ok {
		result["schema"] = "2.0"
		result["body"] = map[string]any{"elements": helpElements}
		return result
	}
...
func renderHelpCardElements(card *core.Card, sessionKey string) ([]map[string]any, bool) {
	tabRows := 0
	panelTitle := "Commands"
	for tabRows < len(card.Elements) {
		actions, ok := card.Elements[tabRows].(core.CardActions)
		if !ok || !isHelpTabActions(actions) {
			break
		}
		if title := selectedHelpTabTitle(actions); title != "" {
			panelTitle = title
		}
		tabRows++
	}
	...
	listRows := 0
	for tabRows+listRows < len(card.Elements) {
		if _, ok := card.Elements[tabRows+listRows].(core.CardListItem); !ok {
			break
		}
		listRows++
	}
	...
	elements := make([]map[string]any, 0, len(card.Elements)-listRows+1)
	for i := 0; i < tabRows; i++ {
		elements = append(elements, renderHelpTabActions(card.Elements[i].(core.CardActions), sessionKey))
	}
	panelElements := make([]map[string]any, 0, listRows)
	for i := tabRows; i < tabRows+listRows; i++ {
		panelElements = append(panelElements, renderHelpListItem(card.Elements[i].(core.CardListItem), sessionKey))
	}
	elements = append(elements, buildCollapsiblePanel(panelTitle, panelElements))
	for i := tabRows + listRows; i < len(card.Elements); i++ {
		switch e := card.Elements[i].(type) {
		case core.CardNote:
			elements = append(elements, renderHelpNote(e))
		case core.CardDivider:
...
func renderHelpTabActions(actions core.CardActions, sessionKey string) map[string]any {
	columns := make([]map[string]any, 0, len(actions.Buttons))
	for _, btn := range actions.Buttons {
		columns = append(columns, map[string]any{
			...
			"vertical_align":   "center",
			"horizontal_align": "center",
			"elements":         []map[string]any{renderHelpButton(btn.Text, btn.Type, btn.Value, btn.Extra, sessionKey, true)},
		})
	}
	columnSet := map[string]any{
		"tag":     "column_set",
...
func renderHelpListItem(item core.CardListItem, sessionKey string) map[string]any {
	return map[string]any{
		...
		"columns": []map[string]any{
			...
			},
			{
				...
				"vertical_align": "center",
				"elements": []map[string]any{
					renderHelpButton(item.BtnText, item.BtnType, item.BtnValue, item.Extra, sessionKey, false),
				},
			},
		},
	}
...
func renderHelpButton(text, btnType, action string, extra map[string]string, sessionKey string, fill bool) map[string]any {
	if btnType == "" {
		btnType = "default"
	}
	valMap := map[string]string{"action": action}
	if sessionKey != "" {
		valMap["session_key"] = sessionKey
	}
	for k, v := range extra {
...
func renderHelpNote(note core.CardNote) map[string]any {
	return map[string]any{
		"tag": "div",
		"text": map[string]any{
			"tag":        "plain_text",
...
func isHelpTabActions(actions core.CardActions) bool {
	if actions.Layout != core.CardActionLayoutEqualColumns || len(actions.Buttons) == 0 {
		return false
	}
	for _, btn := range actions.Buttons {
		if !isHelpTabAction(btn.Value) {
			return false
		}
	}
	return true
...
func isHelpTabAction(action string) bool {
	action = strings.TrimSpace(action)
	return action == "nav:/help" || strings.HasPrefix(action, "nav:/help ")
}

func selectedHelpTabTitle(actions core.CardActions) string {
	if len(actions.Buttons) == 0 {
		return ""
	}
	for _, btn := range actions.Buttons {
		if strings.EqualFold(btn.Type, "primary") && strings.TrimSpace(btn.Text) != "" {
			return btn.Text
		}
	}
	return strings.TrimSpace(actions.Buttons[0].Text)
}
