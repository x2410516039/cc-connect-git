...
func TestPreprocessFeishuMarkdown_SeparatesThematicBreakFromPreviousLine(t *testing.T) {
	input := "**工具 #1: WebSearch**\n---\n```text\nquery\n```"
	out := preprocessFeishuMarkdown(input)
	if strings.Contains(out, "**工具 #1: WebSearch**\n---") {
		t.Fatalf("expected blank line before thematic break to avoid Setext heading, got %q", out)
	}
	if !strings.Contains(out, "**工具 #1: WebSearch**\n\n---\n```text\nquery\n```") {
		t.Fatalf("expected tool progress markdown to preserve code fence with separated thematic break, got %q", out)
	}
}

func TestBuildPreviewCardJSON_NormalizesToolLabelsBeforeThematicBreaks(t *testing.T) {
	input := "🔧 **工具 #13: Bash**\n---\nhttps://example.com/log\n---\n```bash\necho ok\n```"
	content := extractSingleCardMarkdownContent(t, buildPreviewCardJSON(input))
	if strings.Contains(content, "🔧 **工具 #13: Bash**\n---") {
		t.Fatalf("expected tool label to be separated from thematic break, got %q", content)
	}
	if strings.Contains(content, "https://example.com/log\n---") {
		t.Fatalf("expected raw URL to be separated from thematic break, got %q", content)
	}
	if !strings.Contains(content, "🔧 **工具 #13: Bash**\n\n---\nhttps://example.com/log\n\n---\n```bash\necho ok\n```") {
		t.Fatalf("expected preview markdown to preserve content with separated thematic breaks, got %q", content)
	}
}
...
func extractSingleCardMarkdownContent(t *testing.T, raw string) string {
	t.Helper()
	var card struct {
		Body struct {
			Elements []struct {
	...
	if card.Body.Elements[0].Tag != "markdown" {
		t.Fatalf("element tag = %q, want markdown", card.Body.Elements[0].Tag)
	}
