...
func TestRenderCardMap_FoldsHelpListItems(t *testing.T) {
	const sessionKey = "feishu:oc_chat:root:om_root"
	card := core.NewCard().
		Title("Help", "blue").
		ButtonsEqual(
			core.PrimaryBtn("Session", "nav:/help session"),
			core.DefaultBtn("Agent", "nav:/help agent"),
		).
		ButtonsEqual(
			core.DefaultBtn("Tools", "nav:/help tools"),
			core.DefaultBtn("System", "nav:/help system"),
		).
		ListItem("`/new` - New session", "▶", "cmd:/new").
		ListItem("`/switch` - Switch session", "▶", "cmd:/switch").
		Note("Tip").
		Build()

	got := renderCardMap(card, sessionKey)
	if schema := got["schema"]; schema != "2.0" {
		t.Fatalf("schema = %#v, want 2.0", schema)
	}
	body, ok := got["body"].(map[string]any)
	if !ok {
		t.Fatalf("body = %#v, want schema 2.0 body", got["body"])
	}
	elements, ok := body["elements"].([]map[string]any)
	if !ok || len(elements) != 4 {
		t.Fatalf("body elements = %#v, want two tab rows, panel, note", body["elements"])
	}

	for i := 0; i < 2; i++ {
		if tag := elements[i]["tag"]; tag != "column_set" {
			t.Fatalf("tab row %d tag = %#v, want column_set", i, tag)
		}
	}
	tabButton := elements[0]["columns"].([]map[string]any)[0]["elements"].([]map[string]any)[0]
	tabValue := callbackBehaviorValue(tabButton)
	if tabValue["action"] != "nav:/help session" {
		t.Fatalf("help tab action = %#v, want nav:/help session", tabValue["action"])
	}
	if tabValue["session_key"] != sessionKey {
		t.Fatalf("help tab session_key = %#v, want %q", tabValue["session_key"], sessionKey)
	}

	panel := elements[2]
	if tag := panel["tag"]; tag != "collapsible_panel" {
		t.Fatalf("panel tag = %#v, want collapsible_panel", tag)
	}
	if expanded := panel["expanded"]; expanded != false {
		t.Fatalf("panel expanded = %#v, want false", expanded)
	}
	panelElements, ok := panel["elements"].([]map[string]any)
	if !ok || len(panelElements) != 2 {
		t.Fatalf("panel elements = %#v, want two list rows", panel["elements"])
	}
	commandColumns := panelElements[0]["columns"].([]map[string]any)
	commandButton := commandColumns[1]["elements"].([]map[string]any)[0]
	commandValue := callbackBehaviorValue(commandButton)
	if commandValue["action"] != "cmd:/new" {
		t.Fatalf("command action = %#v, want cmd:/new", commandValue["action"])
	}
	if commandValue["session_key"] != sessionKey {
		t.Fatalf("command session_key = %#v, want %q", commandValue["session_key"], sessionKey)
	}

	if tag := elements[3]["tag"]; tag != "div" {
		t.Fatalf("last element tag = %#v, want note text outside panel", tag)
	}
}

func callbackBehaviorValue(button map[string]any) map[string]string {
	behaviors := button["behaviors"].([]map[string]any)
	return behaviors[0]["value"].(map[string]string)
}

func TestRenderCardMap_NonHelpListCardDoesNotFold(t *testing.T) {
	card := core.NewCard().
		ButtonsEqual(
			core.PrimaryBtn("Confirm", "act:/confirm"),
			core.DefaultBtn("Cancel", "act:/cancel"),
		).
		ListItem("Regular row", "Open", "act:/open").
		Build()

	got := renderCardMap(card, "feishu:oc_chat:root:om_root")
	elements, ok := got["elements"].([]map[string]any)
	if !ok || len(elements) != 2 {
		t.Fatalf("elements = %#v, want unmodified action row and list row", got["elements"])
	}
	for _, elem := range elements {
		if tag := elem["tag"]; tag == "collapsible_panel" {
			t.Fatalf("ordinary card should not be folded, got %#v", elements)
		}
	}
}
