...
func TestInteractivePlatform_CardActionPassesCardSenderToHandler(t *testing.T) {
	...
	case receivedMsg := <-msgCh:
		...
		if rc.messageID != "" {
			t.Fatalf("replyContext.messageID = %q, want empty for card command dispatch", rc.messageID)
		}
		if rc.chatID != chatID {
			t.Fatalf("replyContext.chatID = %q, want %q", rc.chatID, chatID)
		}
	case <-time.After(2 * time.Second):
...
func TestInteractivePlatform_LegacyHelpNavDispatchesSlowCommandsAsync(t *testing.T) {
	tests := []struct {
		action string
		want   string
	}{
	...
	for _, tc := range tests {
		t.Run(tc.action, func(t *testing.T) {
			...
			ip.cardNavHandler = func(action, sessionKey string) *core.Card {
				t.Fatalf("legacy help action should dispatch as command, got card nav action %q", action)
				return nil
			}
			...
			if resp != nil {
				t.Fatalf("response = %#v, want nil for async command dispatch", resp)
			}

			select {
			case msg := <-msgCh:
				if msg.Content != tc.want {
					t.Fatalf("message content = %q, want %q", msg.Content, tc.want)
				}
				...
				if rc.messageID != "" {
					t.Fatalf("replyContext.messageID = %q, want empty for async command dispatch", rc.messageID)
				}
				if rc.chatID != "oc_test_chat" {
					t.Fatalf("replyContext.chatID = %q, want oc_test_chat", rc.chatID)
				}
			case <-time.After(2 * time.Second):
				t.Fatal("expected card action message")
			}
...
func newCardActionTestPlatform(t *testing.T) *interactivePlatform {
	t.Helper()
	...
	ip.userNameCache.Store("ou_test_user", "Test User")
	ip.chatNameCache.Store("oc_test_chat", "Test Chat")
	return ip
}
...
func TestInteractivePlatform_HelpCommandActionsDispatchAsMessages(t *testing.T) {
	...
	for _, action := range actions {
		t.Run(action, func(t *testing.T) {
			...
			select {
			case msg := <-msgCh:
				if msg.Content != strings.TrimPrefix(action, "cmd:") {
					t.Fatalf("message content = %q, want %q", msg.Content, strings.TrimPrefix(action, "cmd:"))
				}
				if msg.SessionKey != "feishu:oc_test_chat:root:om_root" {
					t.Fatalf("SessionKey = %q, want callback session key", msg.SessionKey)
				}
				rc, ok := msg.ReplyCtx.(replyContext)
				if !ok {
					t.Fatalf("ReplyCtx type = %T, want replyContext", msg.ReplyCtx)
				...
				if rc.messageID != "" {
					t.Fatalf("replyContext.messageID = %q, want empty for command dispatch", rc.messageID)
				}
				if rc.chatID != "oc_test_chat" {
					t.Fatalf("replyContext.chatID = %q, want oc_test_chat", rc.chatID)
				}
...
func TestInteractivePlatform_HelpNavAndActActionsReturnCardUpdates(t *testing.T) {
	actions := []string{
		"nav:/help session",
		"nav:/help agent",
		"nav:/help tools",
	...
	for _, action := range actions {
		t.Run(action, func(t *testing.T) {
			...
			ip.cardNavHandler = func(action string, sessionKey string) *core.Card {
				actionCh <- action
				if sessionKey != "feishu:oc_test_chat:root:om_root" {
					t.Fatalf("sessionKey = %q, want callback session key", sessionKey)
				}
...
func TestInteractivePlatform_SelectStaticOptionsFallbackPassesReasoningAction(t *testing.T) {
	ip := newCardActionTestPlatform(t)
	...
	ip.cardNavHandler = func(action string, sessionKey string) *core.Card {
		actionCh <- action
		sessionCh <- sessionKey
		return core.NewCard().Markdown("reasoning updated").Build()
	}
	...
	resp, err := ip.onCardAction(&callback.CardActionTriggerEvent{
		Event: &callback.CardActionTriggerRequest{
			...
			Action: &callback.CallBackAction{
				Tag:     "select_static",
				Options: []string{"act:/reasoning 2"},
				Value: map[string]any{
					"session_key": wantSessionKey,
				},
			},
			Context: &callback.Context{
				...
				OpenMessageID: "om_reasoning_card",
			},
		},
	})
	if err != nil {
	...
	select {
	case got := <-actionCh:
		if got != "act:/reasoning 2" {
			t.Fatalf("action = %q, want act:/reasoning 2", got)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("expected card nav handler invocation")
	}
...
func TestBuildPreviewCardJSON_ProgressPayloadFoldsToolEntries(t *testing.T) {
	exitCode := 0
	payload := core.BuildProgressCardPayloadV2([]core.ProgressCardEntry{
		{Kind: core.ProgressEntryThinking, Text: "planning"},
		{Kind: core.ProgressEntryToolUse, Tool: "Bash", Text: "pwd"},
		{Kind: core.ProgressEntryToolResult, Tool: "Bash", Text: "workspace", ExitCode: &exitCode},
	}, false, "Codex", core.LangEnglish, core.ProgressCardStateRunning)
	if payload == "" {
		t.Fatal("BuildProgressCardPayload returned empty payload")
	}
	...
	for _, raw := range elements {
		...
		if strings.Contains(fmt.Sprint(element), "planning") {
			thinkingOutsidePanel = true
		}
	}
	if len(panels) != 1 {
		t.Fatalf("expected one collapsible_panel, got %d in %#v", len(panels), elements)
	}
	panel := panels[0]
	if panel["expanded"] != false {
		t.Fatalf("expected collapsible panel to default collapsed, got %#v", panel["expanded"])
	}
	header, ok := panel["header"].(map[string]any)
	if !ok || header == nil {
		t.Fatalf("expected collapsible panel header, got %#v", panel["header"])
	}
	if _, ok := header["title"]; !ok {
		t.Fatalf("expected collapsible panel header title, got %#v", header)
	}
	nested, ok := panel["elements"].([]any)
	if !ok {
		t.Fatalf("expected collapsible panel elements, got %#v", panel["elements"])
	}
	nestedText := fmt.Sprint(nested)
	if !strings.Contains(nestedText, "Tool") || !strings.Contains(nestedText, "Result") {
		t.Fatalf("expected nested tool label and result markdown, got %#v", nested)
	}
	if !strings.Contains(nestedText, "Bash") || !strings.Contains(nestedText, "workspace") {
		t.Fatalf("expected nested tool content, got %#v", nested)
	}
	if !thinkingOutsidePanel {
		t.Fatalf("expected thinking entry outside collapsible panel, got %#v", elements)
	}
}

func assertCollapsedMarkdownCard(t *testing.T, raw string, wantContent string) {
	t.Helper()
	...
	if len(card.Body.Elements) != 1 {
		t.Fatalf("body elements = %d, want 1", len(card.Body.Elements))
	}
	panel := card.Body.Elements[0]
	if panel.Tag != "collapsible_panel" {
		t.Fatalf("body.elements[0].tag = %q, want collapsible_panel", panel.Tag)
	}
	if panel.Expanded == nil || *panel.Expanded {
		t.Fatalf("body.elements[0].expanded = %#v, want false", panel.Expanded)
	}
	if panel.Header.Title == nil {
		t.Fatalf("body.elements[0].header.title missing")
	}
	if len(panel.Elements) != 1 {
		t.Fatalf("panel elements = %d, want 1", len(panel.Elements))
	}
	if panel.Elements[0].Tag != "markdown" {
		t.Fatalf("panel.elements[0].tag = %q, want markdown", panel.Elements[0].Tag)
	}
	if panel.Elements[0].Content != wantContent {
		t.Fatalf("panel.elements[0].content = %q, want %q", panel.Elements[0].Content, wantContent)
	}
}

func TestBuildPreviewCardJSON_CollapsesFinalAnswer(t *testing.T) {
	assertCollapsedMarkdownCard(t, buildPreviewCardJSON("final answer"), "final answer")
}

func TestBuildReplyContent_CollapsesFinalAnswer(t *testing.T) {
	msgType, body := buildReplyContent("final answer")
	if msgType != larkim.MsgTypeInteractive {
		t.Fatalf("buildReplyContent() msgType = %q, want interactive", msgType)
	}
	assertCollapsedMarkdownCard(t, body, "final answer")
}
...
func TestBuildPreviewCardJSON_NormalTextFallback(t *testing.T) {
	...
	assertCollapsedMarkdownCard(t, cardJSON, "plain progress text")
}
