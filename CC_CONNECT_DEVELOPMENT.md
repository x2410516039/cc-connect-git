...
CC-Connect is a bridge that connects the Codex coding agent with messaging platforms. Users interact with Codex through their preferred messaging app.
...
The `core/` package must remain agnostic. Never write `if p.Name() == "feishu"` or `CreateAgent("otheragent", ...)` in core. Use interfaces and capability checks instead:
...
agent, _ := CreateAgent("codex", opts)
...
// In agent/codex/
func (a *Agent) CLIBinaryName() string  { return "codex" }
...
```bash
# Codex agent + Feishu and Telegram platforms
make build AGENTS=codex PLATFORMS_INCLUDE=feishu,telegram
```
...
Available tags: `no_codex`, `no_feishu`, `no_telegram`,
