...
func TestCmdReasoning_SwitchesEffortAndKeepsSessionID(t *testing.T) {
	p := &stubPlatformEngine{n: "plain"}
	agent := &stubModelModeAgent{}
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	msg := &Message{SessionKey: "test:user1", ReplyCtx: "ctx"}
	...
	s.SetAgentSessionID("existing-session", "test")
	s.AddHistory("user", "hello")
	live := newControllableSession("existing-session")
	...
	case <-live.closed:
	default:
		t.Fatal("expected live session to be recycled after reasoning switch")
	}
	if _, ok := e.interactiveStates[msg.SessionKey]; ok {
		t.Fatal("expected interactive state to be cleared after reasoning switch")
	}
...
func TestHandleCardNav_ReasoningUsesWorkspaceAgentAndSessions(t *testing.T) {
	p := &stubPlatformEngine{n: "plain"}
	...
	ws := e.workspacePool.GetOrCreate(wsDir)
	wsAgent := &stubModelModeAgent{reasoningEffort: "medium"}
	ws.agent = wsAgent
	ws.sessions = NewSessionManager("")
	...
	card := e.handleCardNav("act:/reasoning 3", sessionKey)
	if card == nil {
		t.Fatal("handleCardNav returned nil")
	}
	if wsAgent.reasoningEffort != "high" {
		t.Fatalf("workspace reasoning effort = %q, want high", wsAgent.reasoningEffort)
	}
	if globalAgent.reasoningEffort != "low" {
		t.Fatalf("global reasoning effort = %q, want unchanged low", globalAgent.reasoningEffort)
	}
	if got := ws.sessions.GetOrCreateActive(sessionKey).AgentSessionID; got != "workspace-session" {
		t.Fatalf("workspace session id = %q, want workspace-session", got)
	}
	if got := len(ws.sessions.GetOrCreateActive(sessionKey).History); got != 1 {
		t.Fatalf("workspace history length = %d, want 1", got)
	}
	if got := e.sessions.GetOrCreateActive(sessionKey).AgentSessionID; got != "global-session" {
		t.Fatalf("global session id = %q, want untouched", got)
	}
	if !strings.Contains(card.RenderText(), "high") {
		t.Fatalf("card text = %q, want refreshed reasoning effort", card.RenderText())
	}
}
...
func TestRenderHelpCard_SystemAsyncCommandsUseCommandDispatch(t *testing.T) {
	...
	if _, ok := findCardAction(systemCard, "cmd:/doctor"); !ok {
		t.Fatal("expected /doctor help action to dispatch as command")
	}
	if _, ok := findCardAction(systemCard, "nav:/doctor"); ok {
		t.Fatal("expected /doctor help action not to use synchronous card nav")
	}
}

func TestRenderHelpCard_AllCommandRowsRoute(t *testing.T) {
	e := NewEngine("test", &stubAgent{}, []Platform{&stubPlatformEngine{n: "test"}}, "", LangEnglish)
	sessionKey := "test:user1"

	for _, group := range helpCardGroups() {
		t.Run(group.key, func(t *testing.T) {
			card := e.renderHelpGroupCard(group.key)
			for _, elem := range card.Elements {
				item, ok := elem.(CardListItem)
				if !ok {
					continue
				}

				action := strings.TrimSpace(item.BtnValue)
				if action == "" {
					t.Fatalf("help command %q has empty action", item.Text)
				}

				switch {
				case strings.HasPrefix(action, "cmd:"):
					cmdText := strings.TrimSpace(strings.TrimPrefix(action, "cmd:"))
					fields := strings.Fields(cmdText)
					if len(fields) == 0 || !strings.HasPrefix(fields[0], "/") {
						t.Fatalf("help command %q action = %q, want slash command", item.Text, action)
					}
					cmd := strings.ToLower(strings.TrimPrefix(fields[0], "/"))
					if got := matchPrefix(cmd, builtinCommands); got == "" {
						t.Fatalf("help command %q action = %q does not match a builtin command", item.Text, action)
					}

				case strings.HasPrefix(action, "nav:"), strings.HasPrefix(action, "act:"):
					if got := e.handleCardNav(action, sessionKey); got == nil {
						t.Fatalf("help command %q action = %q did not return a card", item.Text, action)
					}

				default:
					t.Fatalf("help command %q action = %q has unsupported prefix", item.Text, action)
				}
			}
		})
	}
