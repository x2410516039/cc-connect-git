				...
				Projects: []ProjectConfig{
					func() ProjectConfig {
						...
						p.References = ReferenceConfig{
							NormalizeAgents: []string{"codex", "codex"},
		...
		},
		{
			...
			cfg: Config{
				Projects: []ProjectConfig{
					func() ProjectConfig {
						p := validProject("demo")
						p.References.NormalizeAgents = []string{"unknown"}
						return p
					}(),
				},
			},
			wantErr: `projects[0].references.normalize_agents has unsupported value "unknown"`,
		},
		{
...
func TestLoad_ResolvesEnvPlaceholders(t *testing.T) {
 ...
 token = "${TG_TOKEN}"
 chat_id = 12345
 `, filepath.Join("${CC_ROOT}", "state"), filepath.Join("${CC_ROOT}", "repo")))
...
func TestAddPlatformToProject_NewProjectWithAgentTypeAndWorkDir(t *testing.T) {
	...
	err := AddPlatformToProject("sigma", PlatformConfig{Type: "slack", Options: map[string]any{"token": "x"}}, "/sigma", "codex")
	if err != nil {
...
func TestResolveProviderRefs_AgentTypeFiltering(t *testing.T) {
	cfg := &Config{
		Providers: []ProviderConfig{
			{Name: "codex-primary", APIKey: "key-c", AgentTypes: []string{"codex"}},
		...
		Projects: []ProjectConfig{{
			...
			Agent: AgentConfig{
				Type:         "codex",
				ProviderRefs: []string{"codex-primary", "codex-only", "universal"},
			},
		}},
	}
	...
	if len(ps) != 3 {
		t.Fatalf("proj-codex: expected 3 providers, got %d: %+v", len(ps), ps)
	}
	if ps[0].Name != "codex-primary" || ps[1].Name != "codex-only" || ps[2].Name != "universal" {
		t.Fatalf("proj-codex providers = [%s %s %s]", ps[0].Name, ps[1].Name, ps[2].Name)
	}
}
...
func TestResolveProviderRefs_AgentTypesFilter(t *testing.T) {
	cfg := &Config{
		Providers: []ProviderConfig{
			{Name: "codex-primary", AgentTypes: []string{"codex"}},
	...
	if len(ps) != 3 {
		t.Fatalf("expected 3 providers (codex-primary + codex-only + universal), got %v", names)
	}
	if names[0] != "codex-primary" || names[1] != "codex-only" || names[2] != "universal" {
...
func TestResolveProviderRefs_EndpointsOverride(t *testing.T) {
	...
	xp := cfg.Projects[0].Agent.Providers
	if len(xp) != 1 {
...
`
	...
	if len(cfg.Providers) != 1 {
		t.Fatalf("expected 1 global provider, got %d", len(cfg.Providers))
	}

	codexProv := cfg.Providers[0]
