...
func TestValidateManagementAddPlatformTypeAllowsOnlyFeishuLark(t *testing.T) {
	for _, typ := range []string{"feishu", "lark", "FeiShu", "LARK"} {
		if err := validateManagementAddPlatformType(typ); err != nil {
			t.Fatalf("validateManagementAddPlatformType(%q) returned error: %v", typ, err)
		}
	}

	err := validateManagementAddPlatformType("telegram")
	if err == nil {
		t.Fatal("validateManagementAddPlatformType(\"telegram\") returned nil, want error")
	}
	if !strings.Contains(err.Error(), `unsupported platform type "telegram" in Feishu/Lark-only build`) {
